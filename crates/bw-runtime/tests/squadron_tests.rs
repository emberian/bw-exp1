...
async fn test_pvp_war_between_squadrons() {
    ...
    let _sq1_id = result1.squadron.unwrap().id;
...
async fn test_apply_squadron_bonus() {
    ...
    assert!((102..=103).contains(&boosted), "boosted rep was {}", boosted);
    ...
    assert!((102..=103).contains(&boosted), "boosted fame was {}", boosted);
}
