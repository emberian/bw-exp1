...
fn apply_player_filters(player: &bw_core::models::Player, filters: &[EntityFilter]) -> bool {
    for filter in filters {
        if let EntityFilter::NameContains(s) = filter
            && !player.username.to_lowercase().contains(&s.to_lowercase()) {
                return false;
            }
    }
    true
...
async fn handle_get_entity(
    ...
    // Result is Ok(Some(value)) if found, Ok(None) if not found, Err if serialization failed
    let result: Result<Option<serde_json::Value>, serde_json::Error> = match entity_type {
        EntityType::Ship => {
            match state.ships.get(&id) {
                Some(s) => serde_json::to_value(&*s).map(Some),
                None => Ok(None),
            }
        }
        EntityType::Player => {
            match state.player_data.get(&id) {
                Some(p) => serde_json::to_value(&*p).map(Some),
                None => Ok(None),
            }
        }
        EntityType::Mission => {
            ...
            for sector in state.sectors.iter() {
                if let Some(mission) = sector.missions.get(&id) {
                    found = Some(serde_json::to_value(&*mission));
                    break;
                }
            }
            match found {
        ...
        EntityType::Station => {
            ...
            for sector in state.sectors.iter() {
                if let Some(loc) = sector.sector.locations.iter().find(|l| l.id == id) {
                    found = Some(serde_json::to_value(loc));
                    break;
                }
            }
            match found {
        ...
        EntityType::Sector => {
            match state.sectors.get(&id) {
                Some(s) => serde_json::to_value(&s.sector).map(Some),
                None => Ok(None),
            }
        }
    };
    ...
    match result {
        ...
        Err(e) => {
            let _ = tx.send(ServerMessage::Admin(AdminServerMessage::AdminError {
                ...
                message: format!("Failed to serialize {:?} {}: {}", entity_type, id, e),
            })).await;
        }
    }
}
    ...
    match change {
        ...
        StagedChange::EntityUpdate { entity_type, id, changes } => {
            let before = match entity_type {
                EntityType::Ship => state.ships.get(id).and_then(|s| serde_json::to_value(&*s).ok()),
                EntityType::Player => state.player_data.get(id).and_then(|p| serde_json::to_value(&*p).ok()),
                _ => None,
            };
        ...
        StagedChange::SectorUpdate { id, changes } => {
            let before = state.sectors.get(id)
                .and_then(|s| serde_json::to_value(&s.sector).ok());
...
async fn handle_set_breakpoint(
    ...
    if state.debug_controller.set_breakpoint(session_id, bp).is_some() {
...
async fn handle_debug_pause(
    ...
    // Note: No response sent here - the DebugPaused message will be sent
    // when the debugger actually pauses at the next debug event
}
