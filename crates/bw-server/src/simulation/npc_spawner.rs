...
pub fn cleanup_npcs(
    ...
    // Find destroyed NPCs
    let destroyed: Vec<Uuid> = sector
        .ship_ids
        .iter()
        .filter_map(|entry| {
            let ship_id = *entry.key();
            state.ships.get(&ship_id).and_then(|ship| {
                if !ship.is_player_ship && matches!(ship.status, bw_core::models::ShipStatus::Destroyed) {
                    Some(ship_id)
                } else {
                    None
                }
    ...
    for ship_id in destroyed {
        ...
        tracing::debug!("Despawned destroyed NPC {}", ship_id);
    }
