...
impl Persistence {
    ...
    /// Try to send a command, logging on failure.
    fn send(&self, cmd: PersistCmd) {
        if let Err(e) = self.tx.try_send(cmd) {
            match e {
                mpsc::error::TrySendError::Full(cmd) => {
                    tracing::error!(
                        "Persistence channel full, dropping {:?} - potential data loss!",
                        std::mem::discriminant(&cmd)
                    );
                }
                mpsc::error::TrySendError::Closed(_) => {
                    tracing::error!("Persistence channel closed, worker may have crashed");
                }
            }
        }
    }
...
async fn upsert_mission(db: &DatabaseConnection, mission: &Mission) -> Result<(), sea_orm::DbErr> {
    ...
    let model = mission::ActiveModel {
        ...
        credits_reward: Set(mission.credits_reward),
        status: Set(status),
